    ...
    public class CarController : MonoBehaviour
{
    ...
    public void SetInput(float new_steeringInput, float new_motorInput, bool new_breakInput)
    {
        ...
        Debug.Log("Setting break input to: " + new_breakInput);
        if (new_breakInput) breakInput = brakeForce;
        else breakInput = 0f;
    }
    ...
    {
  /*
      ClientControllerInput c =  control.GetComponent<ClientControllerInput>();

        steeringInput = c.steeringInput;
        motorInput = c.motorInput;

        if (c.breakInput) breakInput = brakeForce;
        else breakInput = 0f;
        */
}
